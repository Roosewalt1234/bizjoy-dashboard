...
-- cleaning spaces (sections + utility rooms) per FM contract, feeding the
-- future Cleaning Scheduler. Brand-new module, nothing shared with AMC.

-- Reusable master list of area names (sections + utility/common rooms).
-- Users can add custom entries later; this just seeds the common ones.
...
-- Tower: top level, one per building block within an FM contract/project.
...
-- Floor: belongs to a tower. Auto-generated from floor_count in the app,
-- but stored as real rows so individual floors can be renamed/added/removed.
...
-- Area: a section (Left/Right/Center) or utility room on a floor - or, when
-- floor_id is null, a tower-wide item (e.g. a single staircase serving all floors).
