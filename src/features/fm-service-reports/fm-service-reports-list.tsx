...
import { deleteFmServiceReport, ITEM_SEP } from "./fm-service-reports-api";
import { useFmServiceReportModal } from "./use-fm-service-report-modal";
import { FmServiceReportModal } from "./fm-service-report-modal";

/**
 * FM Service Reports list page. Only triggers the shared FmServiceReportModal.
 */
export function FmServiceReportsListPage({ prefillWo }: { prefillWo?: string } = {}) {
  const qc = useQueryClient();
  const { can } = usePermissions();
  const modal = useFmServiceReportModal();
  ...
  useEffect(() => {
    if (!prefillWo) return;
    modal.openCreate(prefillWo);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [prefillWo]);

  const { data: reports = [], isLoading } = useQuery({
    queryKey: ["fm_service_reports"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("fm_service_reports")
        .select("*")
  ...
  const filtered = useMemo(() => {
    ...
    return (reports as any[]).filter((r) => {
      ...
      if (!q) return true;
      return [r.report_no, r.customer_name, r.technician_name, r.location].some((v) => (v ?? "").toString().toLowerCase().includes(q));
    });
  ...
  async function remove(id: string) {
    try {
      await deleteFmServiceReport(id);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">FM Service Reports</h1>
          <p className="text-muted-foreground">Completion reports for FM work orders, with before &amp; after photos</p>
        </div>
        <div className="flex items-center gap-2">
          <ExportMenu
            filename="fm-service-reports"
            rows={filtered}
            sheetName="FM Service Reports"
            columns={[
              ...
              { key: "completion_type", label: "Completion Type" },
          ...
          {can("service", "add") && (
            <Button onClick={() => modal.openCreate()}>
              <Plus className="h-4 w-4 mr-2" /> New Service Report
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <Card className="p-4"><div className="text-xs text-muted-foreground">Total Reports</div><div className="text-2xl font-bold">{reports.length}</div></Card>
        <Card className="p-4"><div className="text-xs text-muted-foreground">Completed</div><div className="text-2xl font-bold text-emerald-600">{completed}</div></Card>
        <Card className="p-4"><div className="text-xs text-muted-foreground">This Month</div><div className="text-2xl font-bold">{thisMonth}</div></Card>
      </div>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={r.id}>
                ...
                <TableCell>{r.service_type ? <Badge variant="secondary" className="gap-1"><Wrench className="h-3 w-3" />{r.service_type}</Badge> : "—"}</TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1">
                    ...
                    {can("service", "edit") && (
                      <Button size="icon" variant="ghost" onClick={() => modal.openEdit(r)}><Pencil className="h-4 w-4" /></Button>
                    )}
                    {can("service", "delete") && (
                      <AlertDialog>
      ...
      <FmServiceReportModal {...modal.dialogProps} />
...
function ViewReportDialog({ report, onClose }: { report: any | null; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const { data } = await supabase.from("fm_service_report_photos").select("*").eq("report_id", report.id).order("sort_order");
      const rows = data ?? [];
      ...
      if (!cancelled) {
        setPhotos(rows.map((p: any) => ({ ...p, beforeUrl: p.before_path ? urlMap[p.before_path] : null, afterUrl: p.after_path ? urlMap[p.after_path] : null })));
      }
    })();
  ...
  const p = (report.problem_reported ?? "").split(ITEM_SEP);
  const w = (report.work_done ?? "").split(ITEM_SEP);
  const pa = (report.parts_used ?? "").split(ITEM_SEP);
  ...
  return (
    <Dialog open={Boolean(report)} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-w-4xl max-h-[88vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>FM Service Report {report.report_no ? `#${report.report_no}` : ""}</DialogTitle>
        </DialogHeader>
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Field label="Client Representative" value={report.client_representative} />
            <Field label="Completion Type" value={report.completion_type} />
            <Field label="Follow-up Required" value={report.follow_up_required ? "Yes" : "No"} />
          ...
          <Field label="Defects Found" value={report.defects_found} />
          ...
          <div className="space-y-3">
            ...
            {photos.length === 0 ? (
              ...
              <Card key={p2.id} className="p-3 space-y-2">
                {p2.caption && <div className="text-sm font-medium">{p2.caption}</div>}
                <div className="grid grid-cols-2 gap-3">
                  {(["before", "after"] as const).map((side) => {
                    const url = side === "before" ? p2.beforeUrl : p2.afterUrl;
                    return (
          ...
          {report.signature_data && (
            <div className="space-y-1">
              <div className="text-xs text-muted-foreground">Signed by {report.signed_by || "client"}</div>
              <img src={report.signature_data} alt="Signature" className="h-24 border rounded-md bg-background" />
            </div>
          )}
        </div>
      </DialogContent>
