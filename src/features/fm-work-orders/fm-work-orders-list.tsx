...
import { useFmWorkOrderModal } from "./use-fm-work-order-modal";
import { FmWorkOrderModal } from "./fm-work-order-modal";

function matchSlaPolicy(order: any, policies: any[]) {
  return (
    policies.find((policy) => {
      if (policy.contract_id !== order?.contract_id) return false;
      const categoryMatch = !policy.service_category_id || policy.service_category_id === order?.service_category_id;
      const priorityMatch =
        !policy.priority || policy.priority === order?.priority || policy.priority === normalizePriority(order?.priority);
...
/**
 * FM Work Orders list page. Only triggers the shared FmWorkOrderModal - technician
 * assignment and rescheduling live inside the modal, not here.
 */
export function FmWorkOrdersListPage({ focusWo, focusWoId }: { focusWo?: string; focusWoId?: string }) {
  const qc = useQueryClient();
  const navigate = useNavigate();
  const { can } = usePermissions();
  const modal = useFmWorkOrderModal();
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          `
          *,
          fm_contracts:contract_id(id, contract_no, title, customer_name),
  ...
  const { data: slaPolicies = [] } = useQuery({
    queryKey: ["sla-policies-for-fm-work-orders"],
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    const match = (orders as any[]).find((r) => (focusWoId ? r.id === focusWoId : r.wo_no === focusWo));
    if (!match) return;
  ...
  async function logSlaEvent(order: any, eventType: string, patch: Record<string, any>) {
    ...
    const { error } = await (supabase as any).from("fm_work_orders").update(payload).eq("id", order.id);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">FM Work Orders</h1>
          <p className="text-muted-foreground">Raise jobs for technicians before the site visit</p>
        </div>
        <div className="flex items-center gap-2">
          <ExportMenu
            filename="fm-work-orders"
            rows={filtered}
            sheetName="FM Work Orders"
            columns={[
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
        <Card className="p-4"><div className="text-xs text-muted-foreground">Total Work Orders</div><div className="text-2xl font-bold">{orders.length}</div></Card>
        <Card className="p-4"><div className="text-xs text-muted-foreground">Open / Scheduled</div><div className="text-2xl font-bold text-amber-600">{openCount}</div></Card>
        <Card className="p-4"><div className="text-xs text-muted-foreground">In Progress</div><div className="text-2xl font-bold text-blue-600">{inProgress}</div></Card>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={12} className="text-center py-8 text-muted-foreground">No work orders yet.</TableCell></TableRow>
            ) : (
              pageRows.map((r: any) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell><Badge variant="outline" className={statusBadgeClasses(r.response_sla_status)}>{r.response_sla_status ?? "—"}</Badge></TableCell>
      ...
      <FmWorkOrderModal {...modal.dialogProps} />
      <WorkOrderView order={viewing} policies={slaPolicies} onClose={() => setViewing(null)} />
    </div>
  );
}

function WorkOrderView({ order, policies, onClose }: { order: any | null; policies: any[]; onClose: () => void }) {
      ...
      <DialogContent className="max-w-3xl max-h-[88vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Field label="Contract" value={order.fm_contracts?.contract_no ?? order.fm_contracts?.title} />
            <Field label="Asset" value={order.contract_assets?.asset_tag ?? order.contract_assets?.description} />
