-- Phase 13: FM Cleaning Scheduler - recurring cleaning plans per floor/area-type,
-- plus the actual visit log written by cleaners scanning a per-floor NFC tag.
-- This app stays the back-office layer (define schedules, view visit history);
-- the actual scan/check-in UI is a separate app the user is building, writing
-- into these same tables via Supabase Auth + RLS, so ownership (ID column
-- `performed_by_employee_id`) is enforced here in policy, not in app code.

-- Cleaners need real login accounts (for ownership), linked to their HR roster row.
...
-- Each floor's physical NFC tag is encoded with a URL keyed on this token
-- (not the raw floor id) so a lost/cloned tag can be revoked and reissued.
...
-- Recurring plan: e.g. "sections on this floor, daily, 9-11am" / "utility rooms, weekly".
...
-- Actual scan event. Written by the cleaner's own app/session, or by an office
-- user correcting/backfilling a record.
...
-- Office staff with the projects module can see/manage everything; a cleaner can see and
-- create only visits attributed to their own linked employee record (ownership).
...
-- Per-visit checklist rows, one per area on that floor.
